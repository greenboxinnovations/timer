...
//windows
// $check 	= exec("ping -n 1 192.168.0.123");
// $check 	= explode('=', $check);
// $result = trim($check[0]);
//print_r($check[0]);
// if ( $result == 'Minimum' ) {
...
// }else{
// 	$sql = "UPDATE `users` SET `camera` = 0 WHERE `name` = 'admin';";
// }
			...
			function checkCamera(){
				$.ajax({
					...
					success: function(response) {
						...
						if (response == 0) {
							alert('Connect Camera Then Retry Login');
						}else{
							loadLogin();
						}
				    }	
				...
				$.ajax({
					...
					success: function(response) {
						...
						if (json.message == 'Authentication Success') {
							...
							}		// remove password text (incase backpressed)
	...
	<style type="text/css">
		*{padding: 0;margin: 0;}
		body{
			...
			position: relative;
			/*background-image: url("css/splas.png");*/
...
<body>
	<div id="login_well">
		...
		<div id="header">Velocity Timer</div>
